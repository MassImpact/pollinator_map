/**
 * Read values coming from the form - This version does not include the address and the number in the export
 */
...
function geocode(location) {
    var response = UrlFetchApp.fetch("http://maps.googleapis.com/maps/api/geocode/json?address="+escape(location)+"&sensor=false");
 ...
 */
function postToCartoDB(location,displayname,city,description,timestamp,website,name,email,state_province, zip,country,type,size,youtube,image,latitude,longitude) {
     ...
     */
    var query = "INSERT INTO mpgc_users_submitted(location,displayname,city,description,timestamp,website,name,email,state_province,zip,country,type,size,youtube,image,the_geom) VALUES('"+location+"','"+displayname+"','"+city+"','"+description+"','"+timestamp+"','"+website+"','"+name+"','"+email+"','"+state_province+"','"+zip+"','"+country+"','"+type+"','"+size+"','"+youtube+"','"+image+"',"+loc+")";
